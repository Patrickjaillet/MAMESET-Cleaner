...
fn process_zip(path: &Path) -> Result<Vec<(String, u32)>, String> {
    ...
    for index in 0..archive.len() {
        ...
        files.push((entry.name().to_string(), entry.crc32()));
    }
...
fn process_seven_zip(path: &Path) -> Result<Vec<(String, u32)>, String> {
    let archive = sevenz_rust::Archive::open(path).map_err(|e| e.to_string())?;

    let files = archive
        .files
        .iter()
        .filter(|entry| !entry.is_directory)
        .map(|entry| (entry.name.clone(), entry.crc as u32))
        .collect();
...
mod tests {
    ...
    fn detects_ok_missing_corrupted_and_unreferenced_roms() {
        ...
        let rom_set = scan_rom_directory(&tmp, &dat, |_| {}).unwrap();
