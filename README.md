...
- **Precise 1G1R duplicate detection** across all 87 supported systems (not just MAME): a fully reorderable region priority (e.g. World > USA > Europe > Japan, editable in Settings), an optional preferred-language tie-breaker, working status, most recent revision, and official releases are always preferred over prototypes/betas/demos/unlicensed copies. Multi-disc releases are correctly kept as separate discs, never merged
...
- **Automatic integrity check** of the set after a cleanup, with an optional deep-verification mode that recomputes SHA1 (not just CRC32 and size) for every scanned ROM
